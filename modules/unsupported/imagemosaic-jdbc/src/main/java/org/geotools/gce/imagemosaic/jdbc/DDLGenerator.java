...
/**
 * Utility class for generating ddl scripts for a specific spatial extension. The generated scripts
 * should be used as a base for db setup. Manual verification for performance issues is necessary.
 *
 * @author mcr
 */
...
class DDLGenerator extends AbstractCmd {
    ...
    DDLGenerator(
            Config config,
            String spatialTNPrefix,
            String tileTNPrefix,
            int pyramids,
            String statementDelim,
            String srs,
            String targetDir) {
        this.config = config;
    ...
    void writeCreateTables(PrintWriter w) throws Exception {
        for (int i = 0; i <= pyramids; i++) {
            if (tileTNPrefix == null) {
                w.print(
                        dbDialect.getCreateSpatialTableStatementJoined(
    ...
    void writeCreateIndexes(PrintWriter w) throws Exception {
        ...
        for (int i = 0; i <= pyramids; i++) {
            w.print(dbDialect.getCreateIndexStatement(getTableName(spatialTNPrefix, i)));
            w.println(statementDelim);
        }
    }
    ...
    void writeRegister(PrintWriter w) {
        ...
        for (int i = 0; i <= pyramids; i++) {
            w.print(dbDialect.getRegisterSpatialStatement(getTableName(spatialTNPrefix, i), srs));
            w.println(statementDelim);
        }
    }
    ...
    void writeUnRegister(PrintWriter w) {
        ...
        for (int i = 0; i <= pyramids; i++) {
            w.print(dbDialect.getUnregisterSpatialStatement(getTableName(spatialTNPrefix, i)));
            w.println(statementDelim);
        }
    }
}
