 ...
 * file. This gives some of the benificts of using the {@linkplain
 * org.geotools.referencing.factory.epsg.DirectEpsgFactory EPSG database backed authority factory}
...
public class PropertyAuthorityFactory extends DirectAuthorityFactory
    ...
    /**
     * Same as {@link #authority}, but may contains more than one elements in some particular cases.
     */
    private final Citation[] authorities;

    /**
     * The properties object for our properties file. Keys are the authority code for a coordinate
     * reference system and the associated value is a WKT string for the CRS.
     *
     * <p>It is technically possible to add or remove elements after they have been loaded by the
     * constructor. However if such modification are made, then we should update {@link
     * Hints#FORCE_LONGITUDE_FIRST_AXIS_ORDER} accordingly. It may be an issue since hints are
     * supposed to be immutable after factory construction. For now, this class do not allow
    ...
    /**
     * Creates a factory for the specified authorities from the specified file. More than one
     * authority may be specified when the CRS to create should have more than one {@linkplain
     * CoordinateReferenceSystem#getIdentifiers identifier}, each with the same code but different
     * namespace. For example a {@linkplain org.geotools.referencing.factory.epsg.EsriExtension
     * factory for CRS defined by ESRI} uses the {@code "ESRI"} namespace, but also the {@code
     * "EPSG"} namespace because those CRS are used as extension of the EPSG database. Concequently,
     * the same CRS can be identified as {@code "ESRI:53001"} and {@code "EPSG:53001"}, where {@code
     * "53001"} is a unused code in the official EPSG database.
     *
     * @param factories The underlying factories used for objects creation.
     * @param authorities The organizations or party responsible for definition and maintenance of
     *     the database.
     * @param definitions URL to the definition file.
     * @throws IOException if the definitions can't be read.
     * @since 2.4
    ...
    /**
     * The set of codes for a specific type of CRS. This set filter the codes set in the enclosing
     * {@link PropertyAuthorityFactory} in order to keep only the codes for the specified type.
     * Filtering is performed on the fly. Consequently, this set is cheap if the user just want to
     * check for the existence of a particular code.
     */
    ...
    /**
     * The WKT parser for this authority factory. This parser add automatically the authority code
     * if it was not explicitly specified in the WKT.
     */
