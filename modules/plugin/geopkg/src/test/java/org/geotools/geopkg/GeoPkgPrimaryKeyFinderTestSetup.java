...
public class GeoPkgPrimaryKeyFinderTestSetup extends JDBCPrimaryKeyFinderTestSetup {
    ...
    protected void createPlainTable() throws Exception {
        ...
        run(
                "INSERT INTO plaintable VALUES (1, 2, 'one', X'"
                        + setup.toString(gb.point(0, 0))
                        + "')");
        run(
                "INSERT INTO plaintable VALUES (2, 3, 'two', X'"
                        + setup.toString(gb.point(1, 1))
                        + "')");
        run(
                "INSERT INTO plaintable VALUES (3, 4, 'three', X'"
                        + setup.toString(gb.point(2, 2))
                        + "')");
    }
