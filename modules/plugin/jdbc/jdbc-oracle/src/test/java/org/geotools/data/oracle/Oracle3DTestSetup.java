...
public class Oracle3DTestSetup extends JDBC3DTestSetup {
    ...
    protected void createPoint3DTable() throws Exception {
        // set up table
        run(
                "CREATE TABLE point3d (fid int, id int, "
                        + " geom MDSYS.SDO_GEOMETRY, name VARCHAR(20), PRIMARY KEY (fid) )");
        ...
        run(
                "CREATE INDEX POINT3D_GEOM_IDX ON POINT3D(GEOM) INDEXTYPE IS MDSYS.SPATIAL_INDEX" //
                        + " PARAMETERS ('SDO_INDX_DIMS=2 LAYER_GTYPE=\"POINT\"')");

        // insert data
        run(
                "INSERT INTO point3d(fid, id,geom,name) VALUES (point3d_fid_seq.nextval, 0,"
                        + "MDSYS.SDO_GEOMETRY(3001, 4326, NULL, MDSYS.SDO_ELEM_INFO_ARRAY(1,1,1), "
                        + "MDSYS.SDO_ORDINATE_ARRAY(1, 1, 1)), 'p1')");
        run(
                "INSERT INTO point3d(fid, id,geom,name) VALUES (point3d_fid_seq.nextval, 1,"
                        + "MDSYS.SDO_GEOMETRY(3001, 4326, NULL, MDSYS.SDO_ELEM_INFO_ARRAY(1,1,1), "
