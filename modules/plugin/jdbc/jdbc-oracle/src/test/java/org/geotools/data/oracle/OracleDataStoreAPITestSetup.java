...
public class OracleDataStoreAPITestSetup extends JDBCDataStoreAPITestSetup {
    ...
    protected void createLakeTable() throws Exception {
        // set up table
        run(
                "CREATE TABLE lake (fid int, id int, "
        ...
        run(
                "CREATE INDEX LAKE_GEOM_IDX ON lake(GEOM) INDEXTYPE IS MDSYS.SPATIAL_INDEX" //
                        + " PARAMETERS ('SDO_INDX_DIMS=2 LAYER_GTYPE=\"POLYGON\"')");

        // insert data
        run(
                "INSERT INTO lake (fid,id,geom,name) VALUES (lake_fid_seq.nextval, 0,"
    ...
    protected void createRiverTable() throws Exception {
        // set up table
        run(
                "CREATE TABLE river (fid int, id int, "
        ...
        run(
                "INSERT INTO USER_SDO_GEOM_METADATA (TABLE_NAME, COLUMN_NAME, DIMINFO, SRID)"
                        ...
                        + "MDSYS.SDO_DIM_ELEMENT('Y',-90,90,0.5)), 4326)");
        run(
                "CREATE INDEX RIVER_GEOM_IDX ON RIVER(GEOM) INDEXTYPE IS MDSYS.SPATIAL_INDEX" //
                        + " PARAMETERS ('SDO_INDX_DIMS=2 LAYER_GTYPE=\"LINE\"')");

        // insert data
        run(
                "INSERT INTO river (fid, id,geom,river, flow ) VALUES (river_fid_seq.nextval, 0,"
                        + "MDSYS.SDO_GEOMETRY( 2002, 4326, NULL, MDSYS.SDO_ELEM_INFO_ARRAY(1,2,1), "
                        + "MDSYS.SDO_ORDINATE_ARRAY(5,5, 7,4, 7,5, 9,7, 13,7, 7,5, 9,3, 11,3)),"
                        + "'rv1', 4.5)");
        run(
                "INSERT INTO river (fid, id,geom,river, flow ) VALUES (river_fid_seq.nextval, 1,"
                        + "MDSYS.SDO_GEOMETRY( 2002, 4326, NULL, MDSYS.SDO_ELEM_INFO_ARRAY(1,2,1), "
                        + "MDSYS.SDO_ORDINATE_ARRAY(4,6, 4,8, 6,10)),"
                        + "'rv2', 3.0)");
    ...
    protected void createRoadTable() throws Exception {
        // set up table
        run(
                "CREATE TABLE road (fid int, id int, "
                        + "geom MDSYS.SDO_GEOMETRY, name varchar(255), PRIMARY KEY (fid) )");
        ...
        run(
                "INSERT INTO USER_SDO_GEOM_METADATA (TABLE_NAME, COLUMN_NAME, DIMINFO, SRID)"
                        ...
                        + "MDSYS.SDO_DIM_ELEMENT('Y',-90,90,0.5)), 4326)");
        run(
                "CREATE INDEX ROAD_GEOM_IDX ON ROAD(GEOM) INDEXTYPE IS MDSYS.SPATIAL_INDEX" //
                        + " PARAMETERS ('SDO_INDX_DIMS=2 LAYER_GTYPE=\"LINE\"')");

        // insert data
        run(
                "INSERT INTO road (fid,id,geom,name) VALUES (road_fid_seq.nextval, 0,"
                        + "MDSYS.SDO_GEOMETRY( 2002, 4326, NULL, MDSYS.SDO_ELEM_INFO_ARRAY(1,2,1), "
                        + "MDSYS.SDO_ORDINATE_ARRAY(1,1, 2,2, 4,2, 5,1)),"
                        + "'r1')");
        run(
                "INSERT INTO road (fid,id,geom,name) VALUES (road_fid_seq.nextval, 1,"
                        + "MDSYS.SDO_GEOMETRY( 2002, 4326, NULL, MDSYS.SDO_ELEM_INFO_ARRAY(1,2,1), "
                        + "MDSYS.SDO_ORDINATE_ARRAY(3,0, 3,2, 3,3, 3,4)),"
                        + "'r2')");
        run(
                "INSERT INTO road (fid,id,geom,name) VALUES (road_fid_seq.nextval, 3,"
                        + "MDSYS.SDO_GEOMETRY( 2002, 4326, NULL, MDSYS.SDO_ELEM_INFO_ARRAY(1,2,1), "
                        + "MDSYS.SDO_ORDINATE_ARRAY(3,2, 4,2, 5,3)),"
                        + "'r3')");
